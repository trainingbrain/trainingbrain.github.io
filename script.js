document.addEventListener('DOMContentLoaded', () => {
    ...
    function startSequenceMemory() { 
        ...
        function nextSequenceLevel() { 
            ...
            // `currentGameTimer` burada da kullanılmalı ve `setTimeout` çağrısı await içinde olmalı
            new Promise(resolve => { currentGameTimer = setTimeout(resolve, 1000); }).then(() => {
                sequence.push(Math.floor(Math.random() * 9)); 
                showSequence(); 
            });
        }
        async function showSequence() { // async keyword'ü burada kalmalı
            ...
            for (const tileIndex of sequence) { 
                await new Promise(resolve => { setTimeout(resolve, 300); }); // currentGameTimer'ı buraya koymaya gerek yok
                if (tiles[tileIndex]) tiles[tileIndex].classList.add('active'); 
                await new Promise(resolve => { setTimeout(resolve, 600); }); // currentGameTimer'ı buraya koymaya gerek yok
    ...
    function runStroopGame() {
        ...
        let stroopScore = 0; // Yerel tanımlandı
        let stroopTimeLeft = 60; // Yerel tanımlandı
        let currentCorrectColorName; // Yerel tanımlandı
