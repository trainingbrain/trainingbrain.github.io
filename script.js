document.addEventListener('DOMContentLoaded', () => {
    ...
    const langTexts = {
        tr: {
            ...
            // Sıralı Hatırlama
            level: "Seviye", watchSequence: "Sırayı İzle...", yourTurn: "Sıra Sende!", gameover: "Oyun Bitti!", highestLevel: "Ulaştığınız en yüksek seviye:",
            // Stroop
        ...
        en: {
            ...
            // Sequence Memory
            level: "Level", watchSequence: "Watch the sequence...", yourTurn: "Your Turn!", gameover: "Game Over!", highestLevel: "Highest level reached:",
            // Stroop
    ...
    backToMenuButton.addEventListener('click', () => { gameContainer.classList.add('hidden'); selectionScreen.classList.remove('hidden'); gameContent.innerHTML = ''; const modal = document.querySelector('.game-over-modal'); if (modal) modal.remove(); if (currentGameTimer) clearTimeout(currentGameTimer); if (typeof stroopTimer !== 'undefined') clearInterval(stroopTimer); if (typeof nbackGameLoop !== 'undefined') clearTimeout(nbackGameLoop); });
    ...
    function showHangmanLevelSelection() { gameContent.innerHTML = `<h2>Adam Asmaca</h2><h3>${langTexts[currentLang].levelSelect}</h3><p class="game-description">Bu egzersiz, kelime dağarcığınızı, harf tanıma hızınızı ve sözel akıcılığınızı test eder. Doğru kelimeyi bulmak için stratejik düşünme becerilerinizi kullanın.</p><div class="level-selection-container"><button class="level-choice" data-level="basit">Basit</button><button class="level-choice" data-level="orta">Orta</button><button class="level-choice" data-level="zor">Zor</button></div>`; document.querySelectorAll('.level-choice').forEach(button => { button.addEventListener('click', (event) => { initializeHangmanGame(event.target.dataset.level); }); }); }
    ...
    function startStroopTest() { clearInterval(stroopTimer); gameContent.innerHTML = `<div id="stroop-start-screen"><h2>Stroop Testi</h2><h3>${langTexts[currentLang].ready}</h3><p class="game-description">Bu klasik nöropsikolojik test, beyninizin "ketleme" (inhibition) ve "seçici dikkat" yeteneklerini ölçer. Otomatik bir tepki olan okumayı bastırıp, daha fazla çaba gerektiren renk isimlendirme görevine odaklanmanız gerekir.</p><p>Ekranda beliren kelimenin anlamına değil, <strong>yazıldığı renge</strong> odaklan. 60 saniye içinde en yüksek skoru yapmaya çalış!</p><button id="stroop-start-button">${langTexts[currentLang].start}</button></div><div id="stroop-game-area" class="hidden"><div id="stroop-stats"><div id="stroop-timer">Zaman: <span>60</span></div><div id="stroop-score">Skor: <span>0</span></div></div><div id="stroop-word"></div><div id="stroop-choices"></div></div>`; document.getElementById('stroop-start-button').addEventListener('click', runStroopGame); }
    ...
    // ==================================================================
    // ---- N-BACK TESTİ (GELİŞTİRİLMİŞ RİTİM VE ÇOK DİLLİ) ----
    // ==================================================================
    ...
    function showNBackLevelSelection() { gameContent.innerHTML = `<h2>N-Back Testi</h2><h3>${langTexts[currentLang].levelSelect}</h3><p class="game-description">Bu egzersiz, "çalışma belleği" (working memory) adı verilen kritik bir bilişsel işlevi hedefler. Bilgiyi zihninizde aktif olarak tutma, güncelleme ve manipüle etme yeteneğinizi geliştirir.</p><p>Ekranda beliren harfin, seçtiğiniz seviye kadar (N) önceki harfle aynı olup olmadığını takip edin. Eğer harf aynı ise eşleme butonuna basın.</p><div class="level-selection-container"><button class="level-choice" data-level="1">1-Back (Kolay)</button><button class="level-choice" data-level="2">2-Back (Orta)</button><button class="level-choice" data-level="3">3-Back (Zor)</button></div>`; document.querySelectorAll('.level-choice').forEach(button => { button.addEventListener('click', (event) => { nbackLevel = parseInt(event.target.dataset.level); initializeNBackGame(); }); }); }
    function initializeNBackGame() { gameContent.innerHTML = `<h2>${nbackLevel}-Back Testi</h2><div class="nback-container"><div id="nback-stats"><div>${langTexts[currentLang].correct}: <span id="nback-correct">0</span></div><div>${langTexts[currentLang].error}: <span id="nback-errors">0</span></div></div><div id="nback-stimulus-box">...</div><p>Eşleşme gördüğünüzde butona basın.</p><div id="nback-controls"><button id="nback-match-button">Eşleşme</button></div><div id="nback-feedback"></div></div>`; nbackSequence = []; nbackCurrentStep = 0; nbackScore = 0; nbackErrors = 0; canPressButton = false; generateNBackSequence(); document.getElementById('nback-match-button').addEventListener('click', handleNBackMatchPress); nbackGameLoop = setTimeout(runNBackStep, 1000); }
