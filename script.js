document.addEventListener('DOMContentLoaded', () => {
    ...
    // ---- GENEL OYUN SONU FONKSİYONU (WCST hariç) ----
    function showGameOverModal(game, isWin) {
        ...
        // Eğer WCST için çağrılırsa hiçbir şey yapma, çünkü kendi gameOverWcst fonksiyonu var.
        if (content === '') return;
    ...
    function displayHangmanWord() { const wordDisplay = document.querySelector('.word-display'); if (!wordDisplay) return; wordDisplay.innerHTML = hangmanSecretWord.split('').map(letter => `<span class="letter-box">${hangmanCorrectLetters.includes(letter) ? letter : ''}</span>`).join(''); if (wordDisplay.innerText.replace(/\n/g, '') === hangmanSecretWord) { showGameOverModal('hangman', true); } }
    function createHangmanKeyboard() { const keyboard = document.querySelector('.keyboard'); if (!keyboard) return; "ABCÇDEFGĞHIİJKLMNOÖPRSŞTUÜVYZ".split('').forEach(letter => { const keyButton = document.createElement('button'); keyButton.innerText = letter; keyButton.classList.add('key'); keyButton.addEventListener('click', () => handleHangmanGuess(letter, keyButton)); keyboard.appendChild(keyButton); }); }
    function handleHangmanGuess(letter, button) { button.disabled = true; if (hangmanSecretWord.includes(letter)) { hangmanCorrectLetters.push(letter); button.classList.add('correct'); } else { hangmanWrongGuessCount++; button.classList.add('wrong'); updateHangmanFigure(); } displayHangmanWord(); if (hangmanWrongGuessCount === hangmanMaxWrongGuesses) { showGameOverModal('hangman', false); } }
    ...
    function updateStroopTimer() { stroopTimeLeft--; document.querySelector('#stroop-timer span').innerText = stroopTimeLeft; if (stroopTimeLeft <= 0) { clearInterval(stroopTimer); showGameOverModal('stroop', false); } }

    // ==================================================================
    // ---- WISCONSIN KART EŞLEŞTİRME EGZERSİZİ (ÇALIŞAN FİNAL VERSİYON) ----
    // ==================================================================
    ...
    function handleTargetClick(chosenTargetIndex) {
        ...
        // Cevabı ve istatistikleri işle
        if (isCorrect) {
            score++;
            consecutiveCorrect++;
        } else {
            totalErrors++;
            checkPerseveration(chosenTarget);
            consecutiveCorrect = 0;
        }

        // Kural değiştirme ihtiyacını kontrol et
    ...
    function updateUI(isCorrect) {
        ...
        feedbackEl.innerText = isCorrect ? "Doğru" : "Yanlış";
        feedbackEl.className = isCorrect ? 'correct' : 'wrong';
        document.getElementById('wcst-score').innerText = `Doğru: ${score}`;
    }

    function checkPerseveration(chosenTarget) {
        if (currentRuleIndex > 0) {
            const previousRule = WCST_RULES_ORDER[currentRuleIndex - 1];
    ...
    function checkMatch(card1, card2, rule) {
        if (!card1 || !card2 || !rule) return false;
